			...
			if (empty($_GET["id"])) {
				...
				if ($queriedMarket = $mysqliConnection -> query($checkIfMarketExistsQuery)) {
					if ($queriedMarket -> num_rows > 0) {
						$HTMLescapedID = htmlspecialchars($_GET["id"], ENT_QUOTES);
						$productCreationPageHTML = "
						<div id='productCreationContainer'>
							<a href='../../edit/?id={$HTMLescapedID}' id='backLink'>
								<div id='leftArrowCont'></div>
								Go Back
							</a>
						...
						$loginAlert = '
						<div id="alertCont">
							<p id="alertText">No markets owned by you under this ID were found.</p>
						</div>';
					}
				} else {
					$loginAlert = '
