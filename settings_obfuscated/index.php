		...
		if (!isset($_SESSION["loggedIn"]) || $_SESSION["loggedIn"] !== true) {
			if (isset($_COOKIE["logincookie"])) {
				...
				if (strlen(trim($rememberMeID)) === 30) {
					$selectAccountDetailsQuery = "SELECT accountdetails.accountID, accountdetails.username, remembereddevices.tokenHash, accountdetails.email, accountdetails.emailVerified, accountdetails.biography
					FROM accountdetails
					...
					if ($allNeededDetails = $mysqliConnection -> query($selectAccountDetailsQuery)) {
						if ($allNeededDetails -> num_rows > 0) {
							if ($assocNeededDetails = $allNeededDetails -> fetch_assoc()) {
								...
								$escapedUsername = htmlspecialchars($assocNeededDetails["username"], ENT_QUOTES);
								$escapedEmail = htmlspecialchars($assocNeededDetails["email"], ENT_QUOTES);
								$escapedBiography = htmlspecialchars($assocNeededDetails["biography"], ENT_QUOTES);
								$emailIsVerified = $assocNeededDetails["emailVerified"] == true ? "Yes" : "No";
								if (hash_equals($assocNeededDetails["tokenHash"], hash("sha512", $remememberMeToken)) === true) {
			...
			SELECT username, email, emailVerified, biography
			FROM accountdetails
