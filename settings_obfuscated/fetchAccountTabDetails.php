...
if ($mysqliConnection -> connect_error) {
    ...
    if (!empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off") {
            ...
            if ($allNeededDetails = $mysqliConnection -> query($neededDetailsQuery)) {
                if ($allNeededDetails -> num_rows > 0) {
                    if ($assocNeededDetails = $allNeededDetails -> fetch_assoc()) {
                        ...
                        $assocReturn["biographyHTML"] = '<p class="notSelectable" style="font-family: LatoReg, Verdana, sans-serif; font-size: 20px; color: #ffffff;">[Error. Please refresh the page or try again later.]</p>';
                    }
                } else {
        ...
        $assocReturn["biographyHTML"] = '<p class="notSelectable" style="font-family: LatoReg, Verdana, sans-serif; font-size: 20px; color: #ffffff;">[Your connection is insecure and this request could not be processed. Please refresh the page or try again later.]</p>';
    }
}
